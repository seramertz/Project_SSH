...
import (
	...
	"Driver-go/config"
	"Driver-go/localElevator/elevio"
	"strconv"
)

// Checks for unavailable elevators and reassigns eventual orders if an elevator is unavailable.
func ReassignOrders(elevators []*config.ElevatorDistributer, ch_newLocalOrder chan elevio.ButtonEvent) {
	lowestID := 999
...
func AssignOrder(elevators []*config.ElevatorDistributer, order elevio.ButtonEvent) {
	...
	minCost := 99999
	elevCost := 0
	var minElev *config.ElevatorDistributer
	for _, elev := range elevators {
